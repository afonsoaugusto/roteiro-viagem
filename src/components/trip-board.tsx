...
export function TripBoard({ trip, actions }: { trip: Trip; actions: TripAction[] }) {
  ...
  const placesByDay = useMemo(() => {
    const dayOrder = new Map(trip.days.map((d, index) => [d.key, index]));
    const withPlace = items.filter((a) => a.placeName || a.placeUrl);
    const groups = new Map<string, TripAction[]>();
    ...
    return [...groups.entries()]
      .sort((a, b) => (dayOrder.get(a[0]) ?? 999) - (dayOrder.get(b[0]) ?? 999))
      .map(([dayKey, places]) => ({
        dayKey,
        label: trip.days.find((d) => d.key === dayKey)?.label ?? places[0]?.dayLabel ?? dayKey,
        weekday: trip.days.find((d) => d.key === dayKey)?.weekday ?? "",
        places: places.sort((a, b) => a.sort - b.sort),
      }));
  }, [items, trip.days]);

  const placeCount = placesByDay.reduce((sum, group) => sum + group.places.length, 0);
  ...
  const goToPlace = (item: TripAction) => {
    setDay(item.dayKey);
    setPlacesOpen(false);
  };
  ...
  return (
    <main className="mx-auto min-h-dvh max-w-lg pb-28">
      <header className="sticky top-0 z-10 border-b border-[#0f3d3e]/10 bg-[#f4ead8]/90 px-4 pt-[max(0.75rem,env(safe-area-inset-top))] pb-3 backdrop-blur">
        <div className="flex items-start justify-between gap-3">
          ...
          <button
            ...
            className="mt-1 shrink-0 rounded-full border border-[#0f3d3e]/20 bg-white/80 px-3 py-2 text-sm text-[#0f3d3e]"
          >
            Locais
            {placeCount > 0 ? (
              <span className="ml-1 text-[#0f3d3e]/55">{placeCount}</span>
            ) : null}
          </button>
        </div>
      ...
      {placesOpen ? (
        <div className="fixed inset-0 z-20 flex flex-col bg-[#f4ead8]">
          <header className="shrink-0 border-b border-[#0f3d3e]/10 px-4 pt-[max(0.75rem,env(safe-area-inset-top))] pb-3">
            <div className="mx-auto flex max-w-lg items-start justify-between gap-3">
              <div>
                <p className="text-xs tracking-[0.18em] text-[#0f3d3e]/60 uppercase">Menu</p>
                <h2
                ...
                <p className="mt-1 text-sm text-[#163032]/70">
                  {placeCount === 0
                    ...
                    : `${placeCount} ${placeCount === 1 ? "local" : "locais"} · toque para ir ao dia`}
                </p>
              </div>
              <button
                type="button"
                onClick={() => setPlacesOpen(false)}
                className="rounded-full bg-white/80 px-3 py-2 text-sm text-[#0f3d3e]"
              >
                Fechar
              </button>
            </div>
          ...
          <div className="mx-auto w-full max-w-lg flex-1 overflow-y-auto px-4 py-4 pb-[max(1.5rem,env(safe-area-inset-bottom))]">
            {placesByDay.length === 0 ? (
              <p className="mt-10 text-center text-sm text-[#163032]/60">
                Adicione nome ou link de local nas ações para vê-los aqui.
              </p>
            ) : (
              <div className="space-y-6">
                {placesByDay.map((group) => (
                  <section key={group.dayKey}>
                    <h3 className="text-sm font-medium tracking-wide text-[#0f3d3e]/70 uppercase">
                      {group.label}
                      {group.weekday ? ` · ${group.weekday}` : ""}
                    </h3>
                    <ul className="mt-2 space-y-2">
                      {group.places.map((item) => (
                        <li key={item._id ?? `${item.dayKey}-${item.sort}-${item.title}`}>
                          <div className="rounded-3xl bg-white/80 p-4 shadow-sm">
                            ...
                            >
                              ...
                              {item.placeName && item.placeName !== item.title ? (
                                <p className="mt-0.5 text-sm text-[#163032]/70">{item.title}</p>
                              ) : null}
                              {item.time ? (
                                <p className="mt-1 text-xs text-[#0f3d3e]/55">{item.time}</p>
                              ) : null}
                            </button>
                            {item.placeUrl ? (
                              <a
